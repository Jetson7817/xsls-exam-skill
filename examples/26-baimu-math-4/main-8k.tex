...
\ifusebigeightk
  \usepackage[paperwidth=390mm,paperheight=270mm,top=1.35cm,bottom=1.65cm,left=1.7cm,right=1.6cm]{geometry}
\else
  \usepackage[paperwidth=420mm,paperheight=297mm,top=1.5cm,bottom=1.8cm,left=1.8cm,right=1.8cm]{geometry}
\fi
...
\setlist[enumerate]{itemsep=0.1em, topsep=0.1em}
...
\newcommand{\bindingline}{
  \begin{tikzpicture}[remember picture,overlay]
    \draw[densely dotted,line width=0.85pt]
      ...
      ([xshift=10mm,yshift=18mm]current page.south west);
    \node[rotate=90,anchor=south,font=\small]
      at ([xshift=14.5mm]current page.west) {装订线};
  \end{tikzpicture}
}
...
\newcommand{\papertitleblock}{
  \begin{center}
    {\fontsize{20pt}{26pt}\selectfont\bfseries \schoolname\ \examyear\ 年全国高等学校运动训练、武术与民族\par}
    \vspace{0.2em}
    {\fontsize{20pt}{26pt}\selectfont\bfseries 传统体育专业\par}
    \vspace{0.8em}
    {\fontsize{21pt}{28pt}\selectfont\bfseries 单独统一招生考试\quad 数学冲刺卷（四）\par}
  \end{center}
}
...
\newcommand{\scoretable}{
  ...
  \begin{center}
    \begin{tabularx}{0.89\textwidth}{|>{\centering\arraybackslash}m{1.7cm}|*{4}{>{\centering\arraybackslash}X|}}
      \hline
      题号 & 一 & 二 & 三 & 总分 \\
      \hline
      得分 &  &  &  &  \\
...
\newcommand{\spreadpair}[2]{%
  \noindent
  \begin{minipage}[t][0.865\textheight][t]{0.486\textwidth}
    #1
  \end{minipage}\hfill
  \begin{minipage}[t][0.865\textheight][t]{0.486\textwidth}
    #2
